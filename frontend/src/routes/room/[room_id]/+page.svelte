<script>
  ...
  function connectWS(rid) {
    ...
    ws.onopen = () => { wsConnected = true; reconnectAttempts = 0; };
  ...
  function scheduleReconnect(rid) {
    ...
    reconnectAttempts++;
    reconnectTimeout = setTimeout(() => { loadBoard(rid); connectWS(rid); }, delay);
  }
  ...
  function deleteCard(cardId) { send({ type: 'card_delete', card_id: cardId }); editingCard = null; }
  ...
  onMount(async () => {
    ...
    // Read room_id inside onMount — safe from SSR
...
{:else}
  <div class="board-header">
    <div class="board-title">
      <h1>{room.name}</h1>
      <button class="code-btn" on:click={copyCode}>{room.room_code} 📋</button>
    </div>
    <div class="presence">
      {#each activeUsers as u}
        <div class="avatar" title={u.display_name}>{u.display_name[0].toUpperCase()}</div>
      {/each}
  ...
  {#if !wsConnected && reconnectAttempts >= 5}
    <div class="banner">Connection lost. <button class="btn-ghost" on:click={() => { reconnectAttempts = 0; connectWS(room_id); }}>Retry</button></div>
  {/if}
      ...
      <div class="column">
        ...
        >
          {#each col.items as card (card.id)}
            <div class="card" animate:flip={{ duration: 150 }} on:click={() => openEdit(card)} role="button" tabindex="0">
...
{#if editingCard}
  <div class="overlay" on:click={() => editingCard = null} role="button" tabindex="0">
...
<style>
  ...
  h1 { font-size: 1.4rem; }
  .code-btn { background: var(--surface); border: 1px solid var(--border); color: var(--text-muted); font-size: 0.8rem; font-family: monospace; padding: 0.3rem 0.7rem; border-radius: 4px; }
  .presence { display: flex; align-items: center; gap: 0.4rem; }
  .avatar { width: 32px; height: 32px; border-radius: 50%; background: var(--accent); color: white; display: flex; align-items: center; justify-content: center; font-size: 0.85rem; font-weight: 700; }
  .ws-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--text-muted); margin-left: 0.4rem; transition: background 0.3s; }
  .ws-dot.connected { background: var(--success); }
  .banner { background: #7a3030; color: white; padding: 0.6rem 1rem; border-radius: var(--radius); margin-bottom: 1rem; display: flex; align-items: center; gap: 1rem; }
  .board { display: flex; gap: 1rem; align-items: flex-start; overflow-x: auto; padding-bottom: 1rem; }
  .column { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 1rem; min-width: 260px; width: 260px; flex-shrink: 0; }
  ...
  .card { background: var(--card); border: 1px solid var(--border); border-radius: var(--radius); padding: 0.9rem; cursor: grab; transition: border-color 0.15s, box-shadow 0.15s; }
  ...
  .add-card-btn { width: 100%; margin-top: 0.5rem; background: transparent; color: var(--text-muted); font-size: 0.85rem; font-weight: 500; padding: 0.5rem; border-radius: var(--radius); border: 1px dashed var(--border); text-align: left; }
  ...
  .modal { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 2rem; width: 100%; max-width: 420px; display: flex; flex-direction: column; gap: 1rem; }
